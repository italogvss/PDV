  ...
  return (
    <>
      <Button
        ...
        sx={selected.length > 0 ? { borderColor: 'secondary.main', color: 'text.primary' } : undefined}
      >
        {displayLabel}
      </Button>
      ...
      >
        {options.map((option) => (
          <MenuItem key={option} onClick={() => toggleOption(option)} sx={{ gap: 0.5, py: 0.75 }}>
            <Checkbox size="small" checked={selected.includes(option)} sx={{ p: 0, mr: 0.5 }} />
