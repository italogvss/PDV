...
export default function SuppliersPage() {
  ...
  const kpis = useMemo(() => ({ total: suppliers.length }), [suppliers])
    ...
    const filtered = q
      ? suppliers.filter(
          (s) =>
            ...
            (s.document?.includes(search) ?? false),
        )
      : suppliers
      ...
      },
      {
        ...
        renderCell: ({ row }) => (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, height: '100%' }}>
            <Avatar sx={{ width: 34, height: 34, fontSize: 12, fontWeight: 600, flexShrink: 0, bgcolor: 'primary.main' }}>
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <PageKpiGrid>
        <PageKpiCard
          ...
          value={kpis.total}
          badge={{ label: `${kpis.total} cadastrados`, color: 'success', icon: StoreOutlined }}
        />
      </PageKpiGrid>
      ...
      <Card sx={{ overflow: 'hidden', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        >
          <TextField
            ...
            sx={{ m: 1, width: 320, '& .MuiOutlinedInput-root': { backgroundColor: 'surface.sunken' } }}
            slotProps={{
              input: {
                startAdornment: (
        ...
        <DataGrid
          ...
          onRowClick={(params) => navigate(`/fornecedores/${params.row.id}`)}
          slots={{ noRowsOverlay: DataGridNoRowsOverlay }}
          sx={{ cursor: 'pointer' }}
        />
      </Card>

      <SupplierFormModal open={addOpen} onClose={() => setAddOpen(false)} />
    </Box>
  )
}
