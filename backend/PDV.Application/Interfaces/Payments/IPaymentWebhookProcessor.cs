...
// e traduz o payload para o modelo de domínio. Não toca no banco.
public interface IPaymentWebhookProcessor
{
    ...
    // Async: `invoice.paid` pode precisar buscar a fatura de novo no Stripe com `expand` (ver
    // StripeWebhookProcessor) quando o payload do webhook não traz o PaymentIntent.
    Task<PaymentWebhookEvent> ParseAsync(string rawBody, string? signatureHeader);
}
